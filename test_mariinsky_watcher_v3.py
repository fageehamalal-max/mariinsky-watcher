...
def sample_record(
    url="/playbill/playbill/2026/7/31/2_1900/",
    title="Турандот",
    performers=None,
    program=None,
):
    (
        ...
        venue,
        venue_source,
    ) = watcher.parse_mariinsky_url_parts(
        "https://www.mariinsky.ru" + url
    )
...
class MariinskyWatcherV3Tests(unittest.TestCase):
    def test_opera_titles_are_not_ballet_due_to_description_words(self):
        for title in [
            ...
            classification = watcher.classify_event(
                title,
                "opera",
                [
                    "описание содержит "
            ...
            self.assertTrue(
                classification.included_despite_ballet_words
            )

    def test_same_url_title_change_is_replacement(self):
        old = sample_record(
            "/playbill/playbill/2026/7/17/2_1900/",
            "Пиковая дама",
        )
    ...
    def test_participation_splits_participants(self):
        self.assertEqual(
            watcher.split_participation(
                "При участии "
                "Екатерины Семенчук "
                "и Марины Шахдинаровой"
            ),
            [
                "Екатерины Семенчук",
                "Марины Шахдинаровой",
    ...
    def test_mass_program_and_cast_are_separated(self):
        (
            program,
            performers,
        ) = watcher.extract_program_and_performers(
            [
    ...
    def test_responsible_concertmaster_is_not_program(self):
        ...
        self.assertTrue(
            watcher.is_role_line(line)
        )

        self.assertFalse(
            watcher.is_program_line(
                line,
                "Концерт",
            )
        )
            ...
            watcher.extract_performers_from_lines(
                [
                    ...
                    (
                        ...
                        "при этом в ней нет ни одного "
                        "отдельного сольного номера"
                    ),
                ]
            )
        )
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                patch.object(
                    ...
                    "send_message",
                    side_effect=AssertionError(
                        "dry_run must not send Telegram"
                    ),
                ),
            ):
                watcher.main()
