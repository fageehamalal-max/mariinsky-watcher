...
SCHEMA_VERSION = 2
FILTER_VERSION = "V2.6-list-genre-ballet-filter-opera-cast"
...
HEADERS = {
    "User-Agent": "Mozilla/5.0 MariinskyWatcherV2/2.6 (+https://github.com/fageehamalal-max/mariinsky-watcher)",
...
PROGRAM_START_WORDS = [
    "концерт", "симфония", "сюита", "увертюра", "соната", "ноктюрн",
    "реквием", "оратория", "кантата", "рапсодия", "адажио", "танец",
    "прелюдия", "фуга", "квартет", "квинтет", "месса",
]
...
CAST_PLACEHOLDER_PATTERNS = [
    ...
    "будут объявлены позднее", "будет объявлен дополнительно",
...
BALLET_GENRE_LINES = {
    "балет", "балеты", "балет в одном действии", "балет в двух действиях",
    "балет в трех действиях", "балет в трёх действиях", "одноактный балет",
    "одноактные балеты", "вечер балетов", "хореографическая миниатюра",
    "хореографические миниатюры",
}

OPERA_GENRE_LINES = {
    "опера", "оперы", "опера в одном действии", "опера в двух действиях",
    "опера в трех действиях", "опера в трёх действиях", "опера-буффа",
    "драма в музыке", "музыкальная драма",
}
...
CONCERT_TITLE_MARKERS = [
    "концерт", "симфонический вечер", "камерный вечер", "реквием",
    "месса", "оратория", "кантата",
]

MARIINSKY_LIST_BALLET_MARKERS = [
...
MARIINSKY_LIST_OPERA_MARKERS = [
    "опера", "опера-буффа", "драма в музыке", "музыкальная драма",
]

MARIINSKY_LIST_CONCERT_MARKERS = [
...
def marker_in_text(line, marker):
...
def contains_word(line, words):
    return any(marker_in_text(line, w) for w in words)
...
def infer_mariinsky_list_type(text):
    ...
    if any(marker in low for marker in MARIINSKY_LIST_BALLET_MARKERS):
        return "ballet"

    if any(marker in low for marker in MARIINSKY_LIST_OPERA_MARKERS):
        return "opera"

    if any(marker in low for marker in MARIINSKY_LIST_CONCERT_MARKERS):
        return "concert"
...
def is_ballet_event(title, lines):
    ...
    if any(marker in text for marker in hard_ballet_markers):
...
def classify_event(title, lines, list_type=""):
    ...
    if any(has_composer(line) or contains_word(line, PROGRAM_WORDS) or contains_word(line, CONCERT_TITLE_MARKERS) for line in lines[:100]):
...
def looks_like_person_name_single(line):
    ...
    good = 0
    for w in words:
        if re.match(r"^[А-ЯЁA-Z][а-яёa-zА-ЯЁA-Z\-]+$", w):
            good += 1
...
def looks_like_ensemble_phrase(line):
    if is_noise(line) or is_date_line(line) or is_time_line(line):
        return False
    low = canonical_low(line)
    return any(marker_in_text(low, w) for w in ENSEMBLE_WORDS) and not re.search(r"\bдля\b.*\b(оркестр|хор|ансамбль)", low)
...
def is_production_credit_label(label):
    ...
    return any(low.startswith(prefix) for prefix in production_prefixes)
...
def is_performer_line(line):
    ...
    if is_production_credit_label(line):
        return False
...
def is_program_line(line, title):
    ...
    if is_date_line(line) or is_time_line(line) or canonical_low(line) == canonical_low(title):
        return False
    ...
    if "балет" in canonical_low(line) and not (contains_word(line, PROGRAM_WORDS) or has_composer(line)):
        return False
...
def strip_section_prefix(s):
    ...
    s = re.sub(r"^(ИСПОЛНИТЕЛИ|В ПРОГРАММЕ)\s*[—:]\s*", "", s)
    return clean(s)
...
def split_performer_for_compare(item):
    ...
    if is_production_credit_label(item):
        return []

    item = re.sub(r"^(исполнители|состав исполнителей)\s*[—:]\s*", "", item, flags=re.I)
...
def normalized_item_map(items, kind):
    ...
    for raw in items or []:
        ...
        for piece in pieces:
            piece = strip_section_prefix(piece)

            if not piece or is_noise(piece) or is_subscription_line(piece) or is_non_repertoire_info(piece) or is_cast_placeholder_line(piece):
                continue

            if kind == "program":
                if not (has_composer(piece) or contains_word(piece, PROGRAM_WORDS) or "«" in piece or "»" in piece):
                    continue

            compare_key = normalize_compare_key(piece)
            if compare_key and len(compare_key) >= 3:
                result.setdefault(compare_key, clean(piece))
...
def parse_mariinsky_event(url, fallback=""):
    ...
    if is_mariinsky_list_ballet_meta(fallback):
        return None, audit_item(
            ...
            title=fallback_title, venue=venue, date_text=date_text, time_text=time_text, event_type="ballet"
        )
    ...
    if any(canonical_low(marker) in canonical_low(line) for marker in EXTERNAL_STAGE_MARKERS for line in external_stage_check_lines):
...
def philharmonia_page_looks_like_event(title, lines, url):
    ...
    text = canonical_low(" ".join(lines[:140]))
...
def extract_philharmonia_links_from_html(html, base_url):
    ...
    for url in sorted(candidates):
        ...
        out.setdefault(url, {
            "title": text if is_valid_title(text) else "",
            "list_type": "",
            "list_text": text[:1200],
        })
...
def is_mariinsky_ballet_record(record):
    ...
    if record.get("source") != "mariinsky":
        return False
    ...
    return any(marker in low for marker in hard_ballet_markers)
...
def sanitize_events(source, events):
    ...
    if source == "philharmonia_grand":
        return {
            ...
            if not is_bogus_philharmonia_record(rec)
        }
    ...
    if source == "mariinsky":
        return {
            url: rec
            for url, rec in events.items()
            if not is_mariinsky_ballet_record(rec)
        }
...
def is_mariinsky_url_time_guess(record):
    ...
    url = record.get("url", "")
    old_time = clean(record.get("time_text", ""))
    ...
    raw = m.group(1)
    guessed_time = f"{raw[:2]}:{raw[2:]}"

    return old_time == guessed_time
...
def run_self_tests():
    ...
    assert FILTER_VERSION == "V2.6-list-genre-ballet-filter-opera-cast"
    ...
    old_program = dict(old_cast)
    new_program = dict(old_cast)
    old_program["program"] = ["Бетховен. Месса до мажор"]
    new_program["program"] = ["В ПРОГРАММЕ — Бетховен. Месса до мажор"]
    assert change_sections(old_program, new_program) == []
    ...
    assert is_mariinsky_ballet_record(ballet_old)
    ...
    pending = sanitize_pending_messages([
        "СПб филармония\nНазвание: Афиша\nhttps://www.philharmonia.spb.ru/afisha/grand/2026/06/?ev_z=",
        "normal",
    ])
