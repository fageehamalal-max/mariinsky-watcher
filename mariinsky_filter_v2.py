...
SCHEMA_VERSION = 2
FILTER_VERSION = "V2.7.3-state-digest-consistency"
...
SESSION.headers.update({
    "User-Agent": "Mozilla/5.0 MariinskyWatcherV2/2.7.3 (+https://github.com/fageehamalal-max/mariinsky-watcher)",
...
def canonical_philharmonia_url(url):
    ...
    return parsed._replace(
        scheme="https",
        netloc="www.philharmonia.spb.ru",
        path=clean_path,
        query="",
        fragment="",
    ).geturl()
...
def is_labeled_performer_line(line):
    ...
    if is_meaningless_performer_line(line) or is_meaningless_performer_line(label) or is_meaningless_performer_line(rest):
        return False
...
def sanitize_parsed_performers(items):
    ...
    for item in items or []:
        ...
        if is_labeled_performer_line(item) or is_ensemble_performer_line(item) or looks_like_person_name_single(item) or looks_like_person_list(item):
            cleaned.append(item)
...
def split_performer_for_compare(item):
    ...
    if not item or not (is_labeled_performer_line(item) or is_ensemble_performer_line(item) or looks_like_person_name_single(item) or looks_like_person_list(item)):
        return []
...
def parse_mariinsky_event(url, fallback=""):
    ...
    return rec, audit_item(
        ...
        performers_count=len(rec.performers),
        program_count=len(rec.program),
    )
...
def scan_all():
    ...
    scanned["philharmonia_grand"], seen_urls["philharmonia_grand"], failed_urls["philharmonia_grand"] = read_source(
        "philharmonia_grand",
        philharmonia_links,
        parse_philharmonia_event,
        audit,
    )
...
def sanitize_pending_messages(messages):
    ...
    for msg in messages or []:
        ...
        normalized = re.sub(
            r"https://(?:www\.)?philharmonia\.spb\.ru/afisha/grand/\?[^\s]+",
            lambda m: canonical_url(m.group(0)),
            text,
        )
...
def save_state(state):
    ...
    for source in SOURCES:
        state.setdefault("sources", {}).setdefault(source, {})["events"] = sanitize_events(
            source,
            state.get("sources", {}).get(source, {}).get("events", {}),
        )
...
def main():
    ...
    for source in SOURCES:
        ...
        source_messages, reason = build_messages_for_source(
            source,
            old_events,
            scanned[source],
            seen_urls[source],
...
def run_self_tests():
    ...
    assert FILTER_VERSION == "V2.7.3-state-digest-consistency"
    ...
    new_joconda["performers"] = [
        ...
        "Джоконда — Татьяна Сержан",
        "Лаура Адорно — Зинаида Царенко",
        "Энцо Гримальдо — Нажмиддин Мавлянов",
    ]
    ...
    r1 = {
        "source": "philharmonia_grand",
        "venue": "СПб филармония, Большой зал",
        "title": "Органный вечер",
        "date_text": "2 июля 2026",
        "time_text": "20:00",
        "url": "https://www.philharmonia.spb.ru/afisha/grand/?ev_z=347&tag=x",
    }
    r2 = dict(r1)
    ...
    sanitized = sanitize_events("philharmonia_grand", {
        r1["url"]: r1,
        r2["url"]: r2,
    })
    ...
    clean_state = sanitize_events("mariinsky", {
        dirty_state_record["url"]: dirty_state_record,
    })
