...
SCHEMA_VERSION = 2
FILTER_VERSION = "V2.7.4-selftest-fix-state-digest-consistency"
...
SESSION.headers.update({
    "User-Agent": "Mozilla/5.0 MariinskyWatcherV2/2.7.4 (+https://github.com/fageehamalal-max/mariinsky-watcher)",
...
BAD_TITLES = {
    "афиша", "афиша и билеты", "афиша концерты большого зала", "большой зал",
    "главная", "репертуар", "понедельник", "вторник", "среда", "четверг",
    "пятница", "суббота", "воскресенье", "cookie", "cookies",
    "использование cookies", "согласие на использование cookie", "согласие на использование cookies",
}
...
MEANINGLESS_PERFORMER_LINES = {"орган", "исполнители", "исполнитель", "солисты", "солист", "в главных партиях"}
...
def canonical_philharmonia_url(url):
    url = normalize_url(url).replace("&amp;", "&").replace("&amp;amp;", "&")
    parsed = urlparse(url)
    ...
    clean_path = re.sub(r"/+$", "/", parsed.path or "/afisha/grand/")
...
def is_genre_description(line):
    low = canonical_low(line)
    genre_starts = ["опера ", "оперы ", "балет ", "балеты ", "концерт ", "концерты ", "гала-концерт", "симфонический концерт", "камерный концерт"]
...
def is_explanatory_line(line):
    low = canonical_low(line)
    bad_starts = ["к ", "ко ", "посвящается", "к юбилею", "к 120", "к 100", "к 150", "в рамках", "при поддержке", "фестиваль", "звезды белых ночей", "звёзды белых ночей", "виртуальная выставка"]
...
def parse_philharmonia_event(url, fallback=""):
    ...
    fallback_title = fallback_title_from_meta(fallback)
    title = title_from_soup(soup, fallback_title)
    if not is_valid_title(title) or title_key(title) in {"афиша", "афиша и билеты", "большой зал", "афиша концерты большого зала"}:
...
def is_bogus_philharmonia_record(record):
    ...
    parsed = urlparse(url)
    query = parse_qs(parsed.query, keep_blank_values=True)
    if title in {"афиша", "афиша и билеты", "большой зал", "без названия", "афиша концерты большого зала"}:
        return True
...
def run_self_tests():
    ...
    assert FILTER_VERSION == "V2.7.4-selftest-fix-state-digest-consistency"
    ...
    old = {"source": "mariinsky", "url": "https://www.mariinsky.ru/playbill/playbill/2026/7/22/2_2001/", "title": "Бетховен. Торжественная месса", "venue": "Мариинский-2", "date_text": "22 июля 2026", "time_text": "20:01"}
    new = dict(old)
    new["time_text"] = "20:00"
    assert is_parser_time_correction(old, new)
    assert change_sections(old, new) == []

    old_cast = {"source": "mariinsky", "url": "https://www.mariinsky.ru/playbill/playbill/2026/7/22/2_2001/", "title": "Бетховен. Торжественная месса", "venue": "Мариинский-2", "date_text": "22 июля 2026", "time_text": "20:00", "performers": ["Солисты оперы, Хор и Симфонический оркестр Мариинского театра"], "program": []}
    ...
    new_joconda["performers"] = ["Дирижер — Валерий Гергиев", "Джоконда — Татьяна Сержан", "Лаура Адорно — Зинаида Царенко", "Энцо Гримальдо — Нажмиддин Мавлянов"]
    old_joconda["digest"] = "same-as-new-by-bug"
    new_joconda["digest"] = "same-as-new-by-bug"
    msg = format_changed(old_joconda, new_joconda)
    assert "Джоконда — Татьяна Сержан" in msg
    assert "Лаура Адорно — Зинаида Царенко" in msg

    noisy_old = dict(old_cast)
    noisy_new = dict(old_cast)
    noisy_old["performers"] = []
    noisy_new["performers"] = ["Санкт — Петербург", "Зал Стравинского", "опера Николая Римского — Корсакова", "Орган", "Исполнители"]
    ...
    dirty_state_record = {"source": "mariinsky", "url": "https://www.mariinsky.ru/playbill/playbill/2026/7/4/2_1900/", "title": "Джоконда", "venue": "Мариинский-2", "date_text": "4 июля 2026", "time_text": "19:00", "event_type": "opera", "performers": ["Орган", "Исполняется на итальянском языке (сопровождается синхронными титрами на русском языке)", "Исполнители", "Дирижер — Валерий Гергиев"], "program": ["Виртуальная выставка «Однажды в Венеции»"], "digest": "wrong-old-digest"}
